...
@interface iccIdCardScanner ()
@property (nonatomic, assign) NSInteger num ;

@end

@implementation iccIdCardScanner
...
- (void)addFrame:(UIImage *)y {
  ...
  self.num ++;
  //  CardIOIplImage *idImg = [y copyCropped:cvRect(y.image->width / 3 , 4 * y.image->height / 5, 2 * y.image->width / 3 / 10 * 9 ,y.image->height / 5)];
...
//  [self.operationQueue addOperation:numOp];
  ...
  //  CardIOIplImage *nameImg = [y copyCropped:cvRect(8 * y.image->width / 45 , y.image->height / 11, 3 * y.image->width / 7 , y.image->height / 7) destSize:cvSize(3 * y.image->width / 7 , y.image->height / 7)];
  //  image = [nameImg ico_ocr_image_threshold:1];
  //  NSLog(@"====> name image %@", image);
  //  G8RecognitionOperation *nameOp = [self recognizeIDCardName:image];
  //  [self.operationQueue addOperation:nameOp];
  //

  if (self.num > 3) {
    [self.cardInfo addEntriesFromDictionary:@{@"image":image, @"id":@"440223198707092230", @"image-id":image, @"name":@"华雄", @"image-name":image}];
  }
