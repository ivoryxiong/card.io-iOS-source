...
@implementation CardIOIdCardViewController

+ (CardIOIdCardViewController *)cardIOIdCardViewControllerForResponder:(UIResponder *)responder {
  while (responder && ![responder isKindOfClass:[CardIOIdCardViewController class]]) {
...
- (instancetype)initWithIdCardDelegate:(id<CardIOIdCardViewControllerDelegate>)aDelegate
  ...
  if(!aDelegate) {
    NSLog(@"Failed to initialize CardIOIdCardViewController -- no delegate provided");
    return nil;
  }
...
- (id)initWithRootViewController:(UIViewController *)rootViewController {
  [NSException raise:@"Wrong initializer" format:@"The designated initializer for CardIOIdCardViewController is -initWithIdCardDelegate:"];
  return nil;
}
...
- (NSString *)description {
  return [NSString stringWithFormat:@"{delegate: %@; %s%s%s%s%s%s%s%s%s%s}"
          ,self.idCardDelegate
