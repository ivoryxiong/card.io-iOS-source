//
//  CardIOIdCardCameraViewController.m
//  icc
//
...
#import "CardIOIdCardCameraViewController.h"
...
@interface CardIOIdCardCameraViewController () <CardIOViewDelegate>
...
@implementation CardIOIdCardCameraViewController
...
- (void)didReceiveDeviceOrientationNotification:(NSNotification *)notification {
  ...
  switch ([UIDevice currentDevice].orientation) {
    ...
    default:
      if (self.deviceOrientation == UIDeviceOrientationUnknown) {
        newDeviceOrientation = (UIDeviceOrientation)((CardIOIdCardViewController *)self.navigationController).initialInterfaceOrientationForViewcontroller;
      }
      else {
  ...
  if (newDeviceOrientation != self.deviceOrientation) {
    ...
    ((CardIOIdCardViewController *)self.navigationController).initialInterfaceOrientationForViewcontroller = (UIInterfaceOrientation)newDeviceOrientation;
...
- (void)cardIOView:(CardIOView *)cardIOView didScanCard:(CardIOCreditCardInfo *)cardInfo {
  ...
  if (![cardInfo.cardNumber length] || self.context.suppressScanConfirmation
    ...
    if ([cardInfo.cardNumber length]
      ...
      //TODO: constrinct info
      [root.idCardDelegate userDidProvideIdCardCardInfo:nil inIdCardViewController:root];
    }
    else {
      [root.idCardDelegate userDidCancelIdCardViewController:root];
    }
